...
class DeleteJobProfileTool implements ToolContract, ToolMetadataContract
{
    ...
    {
        return 'people.job_profiles.DELETE';
    }

    public function getDescription(): string
    {
        return 'DELETE /people/job-profiles/{id} - Löscht ein JobProfile (soft delete). Wenn an Mitarbeiter zugewiesen: bitte status=archived setzen statt löschen.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            if ($jp->assignments()->exists()) {
                return ToolResult::error('VALIDATION_ERROR', 'JobProfile ist Mitarbeitern zugewiesen und kann nicht gelöscht werden. Setze stattdessen status=archived.');
            }
    ...
    public function getMetadata(): array
    {
        return [
            'category'      => 'action',
            'tags'          => ['people', 'job_profiles', 'delete'],
            'read_only'     => false,
            'requires_auth' => true,
            'requires_team' => true,
            'risk_level'    => 'write',
